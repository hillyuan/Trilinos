...
TEUCHOS_UNIT_TEST(NewmarkImplicitAForm, HarmonicOscillatorDamped_SecondOrder)
{
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if (n == nTimeStepSizes-1) {
      ...
      ftmp.precision(16);
      RCP<const SolutionHistory<double> > solutionHistory = integrator->getSolutionHistory();
      RCP<const Thyra::VectorBase<double> > x_exact_plot;
...
TEUCHOS_UNIT_TEST(NewmarkImplicitDForm, HarmonicOscillatorDamped_SecondOrder)
{
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    // Plot sample solution and exact solution at most-refined resolution
    if (n == nTimeStepSizes-1) {
      std::ofstream ftmp("Tempus_NewmarkImplicitDForm_HarmonicOscillator_Damped_SecondOrder.dat");
      ftmp.precision(16);
      RCP<const SolutionHistory<double> > solutionHistory =
        integrator->getSolutionHistory();
      RCP<const Thyra::VectorBase<double> > x_exact_plot;
    ...
    StepSize.push_back(dt);
    const double L2norm = Thyra::norm_2(*xdiff);
	std::cout << "Dnorm:" << L2norm << std::endl;
    ErrorNorm.push_back(L2norm);
  }

  // Check the order and intercept
  double slope = computeLinearRegressionLogLog<double>(StepSize, ErrorNorm);
  std::cout << "  Stepper = Newmark Implicit d-Form" << std::endl;
  std::cout << "  =========================" << std::endl;
  std::cout << "  Expected order: " << order << std::endl;
  std::cout << "  Observed order: " << slope << std::endl;
  std::cout << "  =========================" << std::endl;
  ...
  std::ofstream ftmp("Tempus_Newmark-Error_HarmonicOscillator_Damped_SecondOrder_DForm.dat");
  ftmp.precision(16);
  double error0 = 0.8*ErrorNorm[0];
  for (int n=0; n<nTimeStepSizes; n++) {
    ftmp << StepSize[n]  << "   " << ErrorNorm[n] << "   "
